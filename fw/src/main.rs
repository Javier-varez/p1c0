...
fn print_boot_args(boot_args: &BootArgs) {
    ...
    println!("Video display: {}", boot_args.boot_video.display);
    ...
    println!("Device tree size: {}", boot_args.device_tree_size);
    println!("Boot flags: {}", boot_args.boot_flags);
    println!("Mem size actual: {}", boot_args.mem_size_actual);
}
